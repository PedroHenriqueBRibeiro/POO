...
DateTime parse_date(const char *s) {
    DateTime t = {0};
    if (sscanf(s, "%d/%d/%d", &t.d, &t.m, &t.y) != 3) {
...
void parse_time_into(DateTime *t, const char *s) {
    if (sscanf(s, "%d:%d", &t->h, &t->min) != 2) {
        fprintf(stderr, "Hora inválida: %s\n", s);
        exit(1);
    }
}
...
void fmt_date(const DateTime *t, char *out) {
    sprintf(out, "%02d/%02d/%04d", t->d, t->m, t->y);
}

void fmt_time(const DateTime *t, char *out) {
    sprintf(out, "%02d:%02d", t->h, t->min);
}

void dbg_dt(const DateTime *t, char *o) {
    sprintf(o, "%02d/%02d/%04d %02d:%02d", t->d, t->m, t->y, t->h, t->min);
}